...
<body>
    <div class="container mt-4">
        <?php if (user()->status == 3) { ?>
            <form action="/accPengantar/updatePengantar/<?= $pengantar['id_pengantar']; ?>" method="post" class="d-inline">
                <input type="hidden" name="status_pengantar" value="1">
                <button type="submit" class="d-none d-sm-inline-block btn btn-sm btn-success shadow-sm">Setujui Surat</button>
            </form>

            <form action="/accPengantar/updatePengantar/<?= $pengantar['id_pengantar']; ?>" method="post" class="d-inline">
                <input type="hidden" name="status_pengantar" value="0">
                <button type="submit" class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm">Batalkan</button> *Klik tombol setujui untuk melakukan validasi tanda tangan kepala sekolah
            </form>
        <?php } ?>
        ...
        <div style="font-size: 15px; font-family:'Times New Roman', Times, serif; height:150px" class="row p-0">
            ...
            <div class="col-6 p-0">
                <?php if ($pengantar['status_pengantar'] == 1) { ?>
                    <img width="200px" src="<?= base_url(); ?>/img/<?= $kepala[0]['ttd']; ?>" alt="">
                <?php } ?>
            </div>
        </div>
        <div style="font-size: 15px; font-family:'Times New Roman', Times, serif;" class="row">
